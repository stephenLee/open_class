...
def multi_lookup(index, query):
    # list(element is string) to string
    s = ''.join(query)
    if s in index:
        return index[s]
    else:
...
def N_gram(content, n):
    result=[]
    for i in xrange(len(content)-(n-1)):
        result.append(content[i:i+n])
    return result

def All_gram(content,n):
    result = []
    for i in range(n):
        result.append(N_gram(content,i+1))
    return result
    
def add_page_to_index(index, url, content):
    contents = content.split()
    # set N_gram, #N=3
    gram = All_gram(contents,3)
    for words in gram:
        for word in words:
            add_to_index(index, ''.join(word), url)
